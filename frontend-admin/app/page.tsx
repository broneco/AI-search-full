...
export default function Home() {
  ...
  const getHeaders = (extraHeaders: Record<string, string> = {}) => {
    ...
    if (!headers["X-User-Id"]) headers["X-User-Id"] = "admin.user";
    if (!headers["X-User-Groups"]) headers["X-User-Groups"] = "User,Management,Admin";
  ...
  // Render assistant response with interactive inline document badges matching frontend-user
  const renderMessageContent = (msg: Message) => {
    if (msg.role !== "assistant") {
      return <div className="whitespace-pre-wrap">{renderFormattedMarkdown(msg.content)}</div>;
    }
              ...
              if (source) {
                return (
                  <button
                    ...
                    title={`${source.title} (${TRANSLATIONS[appLanguage].pageLabel || "Strana"} ${source.page_number || 1})`}
                  >
                    <span>📄</span>
                ...
                return (
                  <button
                    key={idx}
                    onClick={() => handleCitationClick(citeIndex + 1, msg.sources)}
                    className="inline-flex items-center gap-1 mx-1 px-2 py-0.5 rounded-md font-bold text-xs bg-indigo-500/20 text-indigo-300 border border-indigo-500/40 cursor-pointer"
                  >
                    <span>📄</span>
                    <span>[{citeIndex + 1}]</span>
                  </button>
        ...
        {msg.sources && msg.sources.length > 0 && (
          <div className="pt-3 border-t border-white/10 flex flex-wrap gap-2 items-center">
            <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">
              {TRANSLATIONS[appLanguage].citationsTitle || "Zdroje a citace"}:
            </span>
            {msg.sources.map((src, sIdx) => (
              ...
              >
                ...
                <span className="text-[10px] opacity-80 font-mono">({TRANSLATIONS[appLanguage].pageLabel || "Strana"} {src.page_number || 1})</span>
              </button>
            ))}
          </div>
        )}
  ...
  return (
    <div className="flex flex-col flex-1 h-screen overflow-hidden bg-[#090d16] font-sans text-zinc-200">
      ...
      <header
        ...
        className="flex items-center justify-between px-6 py-3 border-b border-white/10 backdrop-blur-md shrink-0 transition-colors duration-300"
      >
        ...
        <div className="flex rounded-lg p-0.5 bg-black/40 border border-white/20">
          <button
            type="button"
        ...
        <div className="flex items-center gap-4">
          {/* Language Switcher */}
          <div className="flex items-center gap-1 px-2 py-1 rounded-xl bg-black/40 border border-white/20">
            <button
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-black/40 border border-white/20 text-white">
            <span className="text-xs font-medium text-white/80">{TRANSLATIONS[appLanguage].userLabel}</span>
            <select
              value={userRole}
              onChange={(e) => setUserRole(e.target.value)}
              className="bg-black/60 border border-white/20 text-xs text-white rounded px-2.5 py-1 focus:outline-none font-semibold cursor-pointer"
            >
          ...
          {userProfile ? (
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-white/15 border border-white/25 text-white">
              <span className="text-xs">👤</span>
              <div className="text-left">
                <p className="text-[11px] font-bold text-white leading-tight">{userProfile.username || userProfile.email}</p>
                <p className="text-[9px] text-white/80 font-semibold">{userProfile.role} | {userProfile.tenant_id}</p>
              </div>
              <button
                onClick={handleLogout}
                className="ml-1 text-xs text-white/70 hover:text-white transition-colors cursor-pointer"
                title="Odhlásit se"
              >
                🚪
              </button>
          ...
          <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/[0.08] border border-white/[0.12] text-xs font-semibold text-white">
            ...
            <span>{apiOnline === null ? TRANSLATIONS[appLanguage].apiConnecting : apiOnline ? "API: Online" : TRANSLATIONS[appLanguage].apiOffline}</span>
          </div>
        </div>
      </header>
            ...
            <aside className={`fixed inset-y-0 right-0 z-50 flex flex-col w-[450px] border-l border-white/[0.08] bg-[#0c1222] shadow-2xl transition-all duration-300 transform ${
              ...
              <div className="flex items-center justify-between p-4 border-b border-white/[0.05] bg-[#0f172a]">
                ...
                <button
                  ...
                  className="text-zinc-400 hover:text-white text-lg transition-colors p-1 cursor-pointer"

...
### Added
- **Dolphin Consulting Parallel Database Support**: Configured isolated database environment support (`dolphin_ai_search` for production/development, `dolphin_ai_search_test` for test execution) to allow parallel runs beside the existing university RAG platform at zero baseline cost.
...
### Changed
- **Universal Branding Transition**: Replaced all university (Jihočeská univerzita, JU, rektorát) mentions, systems (iFIS), and documents (`R_399`, `R_402`, `R_407`, `R_409`) across frontend UI dashboard, API endpoints, backend test suites, and documentation files with Dolphin Consulting branding and generic corporate terminology.
- **New PDF Ingestion Mapping**: Updated `ingest.py` and `full_refresh_ingest.py` to support the 5 new corporate documents (`S-10.150`, `S-10.160`, `S-10.170`, `S-10.300`, `S-10.310`) in the `data/` folder, mapping them to appropriate ACL allowed groups (Management, HR, Finance, User) and custom metadata.
...
- **Isolated Database Testing**: Created `tests/conftest.py` to automatically redirect database connections to a dedicated isolated test database (`dolphin_ai_search_test`) during test execution. This prevents the `pytest` test suite from dropping or wiping the primary development database (`dolphin_ai_search`) when teardown fixtures run.
